package com.hasancankula.fruitninjastarter1;
...
public class Main extends ApplicationAdapter implements InputProcessor {
    ...
    @Override
    public void render() {
        // devamlı çalışacak olan kodları buraya yazarız
        ...
        if(lives>0){
            ...
            for(int i = 0;i<lives;i++){
                batch.draw(apple, i*30f + 60f, Gdx.graphics.getHeight()-50f, 50f, 50f);
            ...
            for (Fruit fruit :fruitArray){
                ...
                switch (fruit.type){
                    case REGULAR:
                        batch.draw(apple, fruit.getPos().x, fruit.getPos().y, Fruit.radius,
                            Fruit.radius);
                        break;
                    case EXTRA:
                        batch.draw(cherry, fruit.getPos().x, fruit.getPos().y, Fruit.radius,
                            Fruit.radius);
                        break;
                    case ENEMY:
                        batch.draw(ruby, fruit.getPos().x, fruit.getPos().y, Fruit.radius,
                            Fruit.radius);
                        break;
                    case LIFE:
        ...
        if(lives<= 0) {
            font.draw(batch, "Cut to play", Gdx.graphics.getWidth()*0.5f,
    ...
    private void addItem(){
        float pos = random.nextFloat() * Math.max(Gdx.graphics.getHeight(),
